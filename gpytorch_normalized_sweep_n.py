...
for index in range(1, len(x_axis_array)): # max_n + 1): # i represents the current value of n
    ...
    X_train = torch.from_numpy(normalize(X_train_original[::i]))
    X_test = torch.from_numpy(normalize(X_test_original[::i]))
    y_train = torch.from_numpy(normalize(y_train_original[::i])) 
    y_test = torch.from_numpy(normalize(y_test_original[::i])) 
